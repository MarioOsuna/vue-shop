...
<div class="email-app-details">
    ...
    <div class="email-detail-header">
        ...
        <div class="email-header-right ml-2 pl-1">
            ...
            <!-- Move to Trash -->
            <feather-icon v-show="$route.params.folder !== 'trash'" icon="TrashIcon" class="ml-75" size="17" @click="$emit('move-email-to-folder', 'trash')" />

            <!-- Show Previous Mail -->
            <feather-icon :icon="$store.state.appConfig.isRTL ? 'ChevronRightIcon' : 'ChevronLeftIcon'" size="17" class="ml-75 cursor-pointer" :class="{'text-muted pointer-events-none': !opendedEmailMeta.hasPreviousEmail}" @click="$emit('change-opened-email', 'previous')" />

            <!-- Show Next Mail -->
            <feather-icon :icon="$store.state.appConfig.isRTL ? 'ChevronLeftIcon' : 'ChevronRightIcon'" size="17" class="ml-75 cursor-pointer" :class="{'text-muted pointer-events-none': !opendedEmailMeta.hasNextEmail}" @click="$emit('change-opened-email', 'next')" />
        </div>
    </div>
